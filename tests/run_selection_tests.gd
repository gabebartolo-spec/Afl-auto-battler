extends SceneTree
## godot --headless --path . --script tests/run_selection_tests.gd
...
func _run() -> void:
	...
	var script = load("res://tests/test_selection.gd")
	if script == null or not script.can_instantiate():
		push_error("Could not load res://tests/test_selection.gd")
		quit(1)
		return
