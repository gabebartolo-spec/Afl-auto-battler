extends SceneTree
## godot --headless --path . --script tests/run_match_game_tests.gd
...
func _run() -> void:
	...
	var script = load("res://tests/test_match_game.gd")
	if script == null or not script.can_instantiate():
		push_error("Could not load res://tests/test_match_game.gd")
		quit(1)
		return
