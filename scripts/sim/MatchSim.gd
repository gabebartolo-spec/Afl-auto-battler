...
func _weighted(group: Array, key: String, power := 2.0):
	if group.is_empty():
		return null
	var weights := []
	for p in group:
		weights.append(powf(maxf(1.0, float(p["attr"][key])), power))
...
func _stoppage(side: int, opp: int, from_bounce: bool) -> void:
	...
	if rng.randf() < float(T["clearance_per_stoppage"]):
		...
		if group.is_empty():
			group = atk.ground
		var mid = _weighted(group, "contested")
		_p(mid, "clearances")
...
func play_chain(side: int, fp: float, from_bounce: bool) -> Dictionary:
	...
	while touches < max_touches:
		...
		if rng.randf() < pressure:
			...
			if tgroup.is_empty():
				tgroup = dfn.ground
			var tackler = _weighted(tgroup, "pressure")
			_p(tackler, "tackles")
...
func resolve_forward50(side: int, fp: float, feeder) -> Dictionary:
	...
	if sgroup.is_empty():
		sgroup = atk.ground
	var shooter = _weighted(sgroup, "goalkicking", 2.4)

	var dgroup := _by_roles(dfn.ground, ["DEF"])
	if dgroup.is_empty():
		dgroup = dfn.ground
	var defender = _weighted(dgroup, "intercept")
...
func run() -> Dictionary:
	...
	for quarter in range(1, 5):
		...
		for i in per_quarter:
			...
			if rng.randf() < float(T["clanger_per_chain"]):
