extends SceneTree
## godot --headless --path . --script tests/run_league_balance_tests.gd
...
func _run() -> void:
	...
	var script = load("res://tests/test_league_balance.gd")
	if script == null or not script.can_instantiate():
		push_error("Could not load res://tests/test_league_balance.gd")
		quit(1)
		return
